...
- [ ] **H1** `python/dtwcpp/_api.py:355-363` — `band` is silently dropped for matrix-free methods (`onebatch`/`clara`/`tadpole`): the matrix-free branch never sets `prob`'s band, so `cluster(data, k, method="onebatch", band=50)` computes UNBANDED distances; this is also a regression for `clara` (band previously honored via the precomputed matrix). Fix: set band on `prob` + rebind (CLI already does this at `dtwc_cl.cpp:864`), or raise `ValueError` if `band != -1` on matrix-free methods. Test: band=5 vs band=-1 must give different distances on warped pairs.
