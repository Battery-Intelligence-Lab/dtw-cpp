...
- [x] Local floors hold: clean native 99/99 (8 capability skips), Python 409 passed (11 capability skips), MATLAB 61/61, all three quickstarts, and documentation gates.
...
- [ ] Write the missing 2026-07-10 session handoff retroactively: `.claude/summaries/handoff-2026-07-10-rc1-omnibus.md` — what `8debf1d` shipped (Tasks 5.8–5.11, Phases 6–7, Tier-1 C++ API, rc1), what the adversarial review found, Phase 8 state.
- [ ] The gate numbers PLAN.md claims for 5.8–5.11/Phase 6/Phase 7 ("ctest 99/99", "Python 409", "MATLAB 61/61", TestPyPI dry-run, wheel smoke) had NO committed run-log at claim time. Re-run each locally and commit verbatim run-logs to `.claude/baselines/2026-07-XX-phase67-floors.md`; where a number disagrees with the PLAN claim (Python is already known to: 407 measured vs 409 claimed), correct the PLAN text and say so.
- [ ] Move/cross-link `benchmarks/baselines/2026-07-10-openmp-profile.md` so all run-logs live under the single `.claude/baselines/` convention.
- [ ] LESSONS.md entries: (a) omnibus commits defeat per-task gating and review — one commit per task; (b) editing a FROZEN contract requires an explicit decision entry, never a silent rewrite; (c) a "regression test" that mirrors the fixed arithmetic in local constexprs instead of calling the fixed code is tautological (test_decode_pair Metal case).
- [ ] CITATIONS.md: add arXiv:2501.19285 (OneBatchPAM), Schultz & Jain 2018, Cuturi & Blondel 2017 if not already present.
...
- 2026-07-10: **Phases 4–7 and 2.0.0rc1 readiness CLOSED.** Final local gates: clean Clang Release ctest 99/99 (8 capability skips), Python 409 passed (11 capability skips), MATLAB 61/61 with 24 OpenMP threads, three executable quickstarts, generated-contract/CLI/error drift checks, Hugo + internal-link gate, clean installed wheel with bundled-HiGHS MIP solve, clean sdist, TestPyPI dry-run, and external-temp CPack archive smoke. Adversarial review fixed explicit-device CPU fallbacks, matrix-free CLARA allocation, Tier-1 C++ API drift, generated-site sdist contamination, build-generator cache collision, stale docs/CLI links, and the mutable Codecov action pin. Final tag/publish and hardware/hosted checks remain explicit operator gates, not falsely claimed local work.
