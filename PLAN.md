...
- [x] Add `URL_HASH` to every CPM URL tarball; pin quickcpplib clone to a SHA.
...
- [ ] **L1** Workflows: SHA-pin the remaining mutable-tag GitHub actions (checkout@v6, upload-artifact@v7, cibuildwheel@v3.4.1, setup-matlab@v3.0.1, actions-hugo@v3 — only codecov is pinned today); pin the Arrow tarball `URL_HASH` (closes the stale "OPEN (Task 0.12)" comment at `cmake/Dependencies.cmake:363` — the Task 0.12 checkbox currently overstates).
