...
- [ ] **M15 (new, confirmed by M14 durability audit)** mmap creation currently publishes a CRC-valid v2 header before the packed region is initialized to NaN and opens an existing path through `creation::if_needed`. A crash can therefore leave zero-filled entries that reopen as computed distances, while concurrent creators can truncate or alias the same cache. Require an explicit initializing/ready publication state, durable data-before-ready ordering, loud rejection of incomplete files, and atomic exclusive creation; pin both a crafted zero-tail cache and synchronized same-path creators.
