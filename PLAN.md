...
- [ ] **H3** `dtwc/algorithms/barycenter.cpp:234-299` — soft-DTW value/gradient recursion has zero non-trivial test coverage (only nx=ny=1, which bypasses the adjoint recursion; a sign/indexing error would silently return the initializer via the line-search guard at :333-343). Fix: finite-difference gradient check on a ≥5×7 non-degenerate pair, band 1e-5 relative, both γ regimes (0.1, 1.0).
