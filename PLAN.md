...
- [ ] **M8** CHANGELOG.md — add missing lines for three behavior changes shipped in `8debf1d`: (a) GPU request warn+CPU-fallback → hard `DeviceError` (contract-authorized), (b) MIP-solver-not-compiled print-and-return → `SolverError`, (c) `dist_by_ind` rebind race fix (Windows 0xc0000409). Fix rc1 section structure: remove the stray `- API contract 2.0 frozen` bullet (:35) and separate the absorbed Phase-4/5 sections from the rc1 heading. Migration guide: document (a) and (b) explicitly (contract §6.1 promised this).
