...
- [ ] **L2** `docs`/`scripts/generate_docs.py` — the lr-core.md content drift check silently degrades to existence-only in CI because its source (`.claude/reports/solver-math-2026-07-06.md`) is gitignored. Fix: commit the canonical derivation into `docs/` (the Phase-4 header already said "move to docs/ in Phase 7") so the drift gate bites everywhere, or make the CI degradation loud in the gate output.
