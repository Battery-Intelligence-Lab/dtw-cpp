...
- [ ] **M14 (new, confirmed by M11 audit)** Mmap distance-cache warm-start validates only N, while the CLI filename depends only on output/name and storage is opened before variant/band configuration. A same-N cache can therefore silently reuse stale distances for different data, bands, missing-data policy, or DTW variants. Add versioned cache metadata/fingerprint validation (or a collision-proof key), fail loudly on mismatch, and test every configuration dimension before any further resume claim.
