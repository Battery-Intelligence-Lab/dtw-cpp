...
- [ ] **M16 (new, confirmed by M13 audit)** CLI `--n-init` is parsed and stored but PAM runs exactly once; Python `DTWClustering(device="hpc")` also drops `n_init` before the remote CLI. Execute deterministic `seed+i` restarts with best-objective retention and overflow/range guards, then propagate `n_init` and the resolved seed through `_hpc`, the runner, submission shell, SLURM export, and final command; pin the seed-42 cost-24 versus seed-43 cost-20 fixture end to end.
