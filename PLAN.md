...
- [ ] **H2** `python/dtwcpp/sklearn.py:266-281` — `__sklearn_tags__` never sets `tags.input_tags.pairwise`; sklearn ≥1.6 ignores the legacy `_more_tags` `pairwise` flag, so `metric="precomputed"` estimators get row-sliced (not row+column-sliced) in `cross_val_*`/`GridSearchCV` → silently wrong CV. Fix: `tags.input_tags.pairwise = (self.metric == "precomputed")`; add `check_estimator` on a precomputed instance (use `expected_failed_checks` where a check requires raw input).
...
- [ ] **H4 (governance)** `docs/api-contract-2.0.md` — the FROZEN contract was edited non-additively (MATLAB Tier-1 method set narrowed; C++ `device="hpc"` re-scoped to throwing "beta") and the STATUS clause requiring a PLAN.md decision entry for changes was itself deleted. Fix: restore a governance clause to the STATUS line; add dated Decision-log entries below authorizing (or reverting) both scope changes. Do not silently re-widen — decide and record.
...
- 2026-07-10 (review): Phase 8 added; the final 2.0.0 tag is gated on Phase 8 CLEAN, not on rc1 as previously implied. The `8debf1d` contract edits (MATLAB Tier-1 method narrowing, C++ HPC throwing-beta) are PROVISIONAL until Task 8.1-H4 records or reverts them here.
