...
- [ ] **M1** `python/dtwcpp/__init__.py:123-145` — `gpu:N` alias rejected by Python but accepted by C++ (`env.cpp:304`), violating contract §6; also `_resolve_device` discards the ordinal on the Metal branch. Fix parse + parity test against the C++ accepted-alias list.
