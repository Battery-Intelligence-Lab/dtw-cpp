...
- [ ] **L6** Barycenter hot path: `align_squared` heap-allocates an nx·ny vector per call, N·k times per iteration, fully serial — hoist to a reusable buffer (buffer > thread_local >> heap rule) and parallelize the per-cluster loop if the no-op oracle stays digit-identical (assignment order must not change results; verify).
