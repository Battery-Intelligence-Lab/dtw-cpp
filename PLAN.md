...
- [ ] **M19 (new, confirmed by M16 audit)** Python `DTWClustering(device="hpc")` still silently replaces non-default `max_iter`, `variant`, every exposed variant parameter, `mv_mode`, `missing_strategy`, and `metric` with remote CLI defaults. Carry the complete distance/clustering configuration through `_hpc`, the runner, wrapper positionals, SLURM exports, and the final command; add a real CLI missing-strategy option or reject unsupported use before submission, and require metric/backend compatibility before any remote side effect. Pin every non-default at the executable final-command boundary while proving defaults remain unchanged.
