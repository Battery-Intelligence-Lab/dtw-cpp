...
- [ ] **M20 (new, confirmed by M17 audit)** Benders temporarily sets `Problem::N_repetition=1` for its Lloyd warm start but restores only method/medoids/labels, corrupting caller-visible configuration after return. Diff every caller-visible `Problem` field before/after success and forced-throw paths, then use an exception-safe guard to restore every temporary mutation without undoing the final clustering result.
