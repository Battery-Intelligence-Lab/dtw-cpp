...
- [ ] **M3** `dtwc/algorithms/barycenter.cpp:55-72` — barycenter silently ignores the Problem's configured DTW variant/band/metric (checks only `ndim != 1`); a Problem set to MSM/TWE/banded/L1 gets unbanded squared-L2 barycenters with no error. Fix: reject non-Standard variant / non-default band / non-default metric at entry with `InvalidInput` (the 5.5/5.6 eager-reject precedent).
