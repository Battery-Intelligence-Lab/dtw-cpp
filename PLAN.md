...
- [ ] **M17 (new, confirmed by M13 audit)** Tier-1 Lloyd `kmedoids` still initializes through the legacy global engine, and Gurobi/HiGHS MIP warm starts call unseeded FastPAM. Plumb invocation-local seeds through Lloyd and both solver warm starts so call order cannot alter results or time-limited trajectories; preserve the explicitly legacy unseeded Tier-2 overload and add global-engine non-consumption regressions.
