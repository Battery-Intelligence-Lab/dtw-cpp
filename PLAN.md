...
- [ ] **M10** `tests/unit/test_decode_pair.cpp` Metal `pair_offset` test is tautological (STATIC_REQUIREs on local constexpr mirrors — passes even if `metal_dtw.mm` is reverted). Replace with a host-side test that exercises the actual chunk-offset arithmetic path (or the real decode function) so reverting the widening fails the test.
