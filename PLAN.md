...
- [ ] **M12** `dtwc/api.cpp:302-312` + `_api.py` — `method="auto"` resolves to `clara` at N>5000 then throws on `device="gpu"`. `auto` must resolve to a device-compatible method (e.g. pam on GPU) instead of resolving into an error; same logic is duplicated in both languages — fix both, add a shared test case.
