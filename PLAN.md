...
- [ ] **L4** `tests/matlab/test_test_api.m:62-74` — the registered serial-honesty band (`available=false`, reason names the sequential build) was replaced by the OpenMP-engagement test; restore it as a SEPARATE case so both build flavors keep coverage.
