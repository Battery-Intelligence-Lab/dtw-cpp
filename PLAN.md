...
- [ ] **M11** `dtwc/dtwc_cl.cpp:845` — tadpole classed "matrix-free" skips mmap, but TADPole's exact/fallback path still lazily allocates the dense N² matrix → OOM risk at the large-N regime where mmap existed on llfio builds. Fix: restore mmap eligibility for tadpole, or guard with a loud size check + documented threshold.
