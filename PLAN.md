...
- [ ] **M4** `barycenter.cpp:204-209` — the "SSG" update drops Schultz & Jain's `2·count_i` multiplicity (a diagonally-preconditioned variant with the same fixed points). Either align with the cited paper or rename/document the deviation in code + docs — the citation must match the implementation.
