...
- [ ] **M6** Hidden `[50k]` OneBatchPAM test uses length-1 series — a degenerate oracle (DTW ≡ |Δscalar|; violates the validate-on-non-degenerate lesson) and its "≤10% of matrix" band is trivially met at 0.51%. Fix: non-degenerate lengths (e.g. 64–128, warped), re-register the band, commit the run-log.
