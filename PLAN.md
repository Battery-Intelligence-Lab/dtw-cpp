...
- [ ] **M28 (new, confirmed by M19 security audit)** Public HPC job names, pre-staged input paths, method/band/skip values, and wrapper positionals reach a comma-delimited `sbatch --export` string embedded in a remote shell command without a complete safe grammar or shell quoting. Spaces/commas corrupt exports and metacharacters in a crafted name/path can become remote syntax. Validate the full submission envelope before run-directory, transfer, or runner side effects; reject unsafe Slurm-export bytes with actionable messages; construct the remote command from shell-quoted arguments; and run the real wrapper against an executing fake-SSH boundary proving no extra command can run while safe paths still submit unchanged.
