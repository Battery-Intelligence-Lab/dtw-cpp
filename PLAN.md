...
- [ ] **M9 (new, confirmed by M7 audit)** `one_batch_pam.cpp` uses `relative_tolerance * max(1, estimated_cost)`, so it is not relative below cost 1 and can reject large fractional improvements. Registered counterexample: `[0,.01,.02,1]`, k=2, full uniform batch, seed 0, one sweep, tolerance .2; gain .01 improves .03→.02 (33.3%), upstream threshold .006 accepts it, current .2 rejects it. Fix and regression as a separate commit.
