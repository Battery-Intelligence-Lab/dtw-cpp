...
- [x] **M20 (new, confirmed by M17 audit)** Benders now scopes its Lloyd warm start behind an exception-safe state guard that snapshots before mutation and restores method, restart count, iteration count, medoids, and labels without allocation during unwind. Broad caller-state snapshots pass after both a successful exact solve and a forced post-Lloyd output failure; the success path still exposes the final Benders labels/medoids. The full HiGHS MIP suite and a no-solver compile/seed gate pass.
...
- 2026-07-10 (Task 8.1-M20): **Benders warm-start state made transactional.** A scoped guard restores every field temporarily changed by the nested Lloyd solve on normal return and stack unwind, while the exact solver's final labels and medoids remain caller-visible. Success and forced post-Lloyd failure snapshots, the full HiGHS MIP suite, and a no-solver build gate pass. Evidence: `.claude/baselines/2026-07-10-phase8-remediation.md` (M20).
