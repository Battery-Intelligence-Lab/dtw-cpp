...
| X-15 | `DTWC_FP_MODEL=strict\|fast`; conformance pinned under `strict`; FP flags `PRIVATE`. **Declare it as a CACHE variable** (`set(... CACHE STRING ...)`, not a plain `set`): X-26's machine record harvests every non-INTERNAL `DTWC_*` entry from `CMakeCache.txt`, so a cache variable lands in benchmark records automatically and a plain one is invisible to them | W0 |
...
| X-24 | PMU counters from the Google Benchmark we already fetch: `BENCHMARK_ENABLE_LIBPFM` + libpfm4 (MIT) on a bare-metal Linux leg — the only route to the O-22 artefact; GitHub runners and macOS cannot serve it (no `perf_event_open`; macOS needs `xctrace`) | W0 (option), R5 (artefact) |
...
| V-5 | X-24 | libpfm4 PMU counters need **bare-metal Linux**. WSL2 usually does not expose `perf_event_open` PMU access, so treat WSL as unproven until tried; a cluster node is the likelier host. |
