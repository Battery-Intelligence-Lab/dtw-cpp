...
| X-05 | **DONE 2026-09-22.** New `dtwc/system_memory.hpp` holds the declaration; `system_memory.cpp` includes that instead of `DataLoader.hpp`, which it had been pulling in for a single line. That was the **only upward include edge in the tree not pointing at `Problem.hpp`**, so the count moves 18 → 17 — the W1 gate's number, reached at the start of the wave rather than the end. `DataLoader.hpp` now includes the new header and re-exports the name, so every existing consumer is unaffected; the new header takes no platform headers, keeping `<windows.h>` out of a header that reaches everything through `<dtwc/dtwc.hpp>`. `scripts/repo_map.py` ranks it `base` alongside `system_memory.cpp` | W1 |
...
| X-12 | **DONE 2026-09-22** (with C-21a). `dtwc::randGenerator` moved to a new `dtwc/random_engine.hpp`, so `settings.hpp` — reached by ~40 translation units — no longer pulls `<random>` for one object almost none of them use. The public name is unaffected: `dtwc/dtwc.hpp` includes the new header, and the 2.0 API contract lists `dtwc::randGenerator` under the umbrella, not under `settings.hpp`. Only code including `settings.hpp` *directly* and relying on it transitively must now include `random_engine.hpp` — one such site in the library (`initialisation.cpp`). Behaviour unchanged: same engine, same seed 29. **C-21a done with it**: `<iostream>` is out of `settings.hpp` too. That removal carried a standing caveat — the header itself recorded that it could not be build-proven from the canonical gate, naming five translation units relying on it transitively, two of them in `benchmarks/` and `examples/`, which are OFF in every configured gate. Two already included it; the other three now do, and the removal was **proven by a build with `DTWC_BUILD_BENCHMARK=ON -DDTWC_BUILD_EXAMPLES=ON`**, which is exactly the evidence the caveat asked for. `<string>` stays: its note claims 24 dependent TUs, a bigger job and not what this row asks | W1 |
...
| X-16 | **DONE 2026-09-22.** `scripts/check_typed_throws.py`, registered as `check_typed_throws` (pure Python, so unlike the codegen report it gates every toolchain). The row's "256 today" was already folklore: the measured count is **259** across 135 files, which is the point of putting a number under a script. Counting is defined rather than grepped — comments, strings, raw strings and character literals are stripped first, so a `throw std::` in a `///` comment does not count (there is exactly one, `dtwc_cl.cpp:335`). Two stripper bugs were found by *disagreeing with grep* rather than by reasoning, and both under-counted silently: `dtwc/metal/metal_dtw.mm` embeds its Metal shader as a raw string `R"(...)"`, and the apostrophes in that shader's own comments (`thread's`, `row's`) were being read as opening character literals that swallowed the rest of the file — hiding **all 18** of its throw sites. A quote now opens a character literal only when what follows actually looks like one. Discrimination proven: adding one `throw std::` gives `untyped=260 ceiling=259 verdict=FAIL` and exit 1 | W1 |
...
| X-27 | **IMPLEMENTED 2026-09-22 — registered band FALSIFIED, decision is Volkan's.** Eigen is gone: `ScratchMatrix` now owns a grow-only, uninitialised `unique_ptr<T[]>` buffer (both Eigen properties preserved deliberately — `std::vector::resize` would value-initialise on every `thread_local` resize in `dtw_kernel.hpp:221`); `to_full_matrix` returns `std::vector<double>`, which **removes** an N×N copy because both binding callers already memcpy'd into one; the `mip/` link was dead (0 includes). CMake, the CPM pin, the supply-chain registry and the MPL-2.0 §3.2 source offer in `THIRD_PARTY_LICENSES.md` are all removed — the wheel now carries no copyleft obligation. **But the band failed.** `BM_dtwFull` is consistently ~5% slower (+5.36/+5.46/+4.63/+5.08% at 100/500/1000/4000; within-set spread 0.15%, so not noise), against a registered ±5%. Two attempts, as the rule allows: the second refuted its own hypothesis (making indices signed like Eigen's changed nothing), so **the cause is unidentified**. Ruled out: value-initialisation, repeated allocation, index signedness. Not examined: allocation alignment, Eigen's resize fast path. The trade — lose the only MPL obligation, pay ~5% on the DTW hot path — is a product decision, and the 5% is laptop wall-clock, which this project's own rule calls advisory until a PMU host confirms it (V-5). Evidence: `.claude/baselines/2026-09-22-x27-drop-eigen-band.md` | W1 |
...
| C-11 | **DONE 2026-09-22.** The foundation now lives in `dtwc/base/`, so the folder a header sits in matches the layer the checker assigns it. Moved (scope confirmed with Volkan): `error`, `settings`, `missing_utils`, `parallelisation`, `timing`, `env.{hpp,cpp}`, `system_memory.{hpp,cpp}`, plus `random_engine.hpp` created earlier today by X-12. `types/` and `enums/` stay where they are — already their own directories, already ranked base — and the four foundation files sitting under `core/` were left for a later row rather than churning `core/` here. **Forwarding headers at the eight old header paths warn via `#pragma message`** and will go next release; the two `.cpp` files moved without forwarders, since a translation unit is not an include target. All 97 internal includes were repointed first, so the warning can only fire for code outside this repo — **proven by a build with benchmarks and examples ON showing zero deprecation messages**, having first caught 48 that the initial pass missed because they were angle-bracket (`#include <parallelisation.hpp>`) rather than quoted. The forwarder itself was compiled standalone to confirm it both resolves and warns. Two gate scripts read moved paths and were repointed (`check_docs_contract.py`, `test_f22_cpp_deprecation_mutations.py`); `docs/content/method/missing-data.md` now shows the new include path. Layer report: `base` 32 files, nothing UNASSIGNED, upward edges still 17 | W1 |
| C-12 | **DONE 2026-09-22.** `dtwc::solver`'s sparse-matrix helpers and tolerances now live in `dtwc/mip/solver_types.hpp`, one file where there were two (`types/element_types.hpp` + `types/types_util.hpp`, the first including the second, 102 lines between them). The row's premise was measurable and is confirmed: `ninja -t deps` says **97 of 297 translation units** compiled these headers, because `utility.hpp:17` pulls `types/types.hpp`; after the move **1 of 297** does. `dtwc/types/` keeps `Range` and `Index`, which is what a base-layer `types/` should hold. **No forwarding header at the old paths** — unlike C-11 — because `types/` ranks `base` (0) and `mip/` ranks 4, so a forwarder would be a `base` -> `mip` upward edge, i.e. the exact coupling the row removes; the break is recorded in `CHANGELOG.md` instead. It is a small one: the names are absent from `docs/api-contract-2.0.md`, and `dtwc.hpp` has never included anything from `mip/`, so they were only ever reachable by accident. The missing `<cmath>` is added; it was not a live break on libc++, where `<cmath>` arrives at include depth 7 from a standard header, which is exactly the kind of accident IWYU hygiene exists to stop. Gate: build green with benchmarks and examples ON, 133 tests / 128 pass (the five V-6 floors), four gate scripts green, `UPWARD_EDGES=17` unchanged, `base` 32 -> 30 files and `mip` 18 -> 19. **Found in passing:** 6 of the 8 moved declarations (`Element`, `Coordinate`, `CompElementIndices`, `CompElementValuesAndIndices`, `ColumnMajor`, `isFractional`) have **no consumer anywhere in the repo** — only `Triplet` and `RowMajor` are used, by `mip_Highs.cpp:119,142`. Deleting them is a separate decision, proposed as **D-21** | W1 |
...
| D-21 | C-12 moved eight `dtwc::solver` declarations into `dtwc/mip/solver_types.hpp` and found that **six have no consumer in the repo**: `Element`, `Coordinate`, `CompElementIndices`, `CompElementValuesAndIndices`, `ColumnMajor`, `isFractional` (and `isAround`, reached only by the unused comparator). Only `Triplet` and `RowMajor` are used, both in `mip_Highs.cpp`. They are the residue of a sparse-matrix assembly path that HiGHS's own API replaced. Deleting them is standing rule 10's "provably unreachable" case, and they are absent from `docs/api-contract-2.0.md` — but they shipped in v1.0.0, so an external MIP integration could name them | delete in W1, or keep as `[[deprecated]]` until the 2.0 tag; recommendation: delete, since C-12 already stopped them being reachable from the umbrella header |
