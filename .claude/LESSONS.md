...
- **On a CACHED distance matrix, k-medoids SWAP is memory-bound, so the O(N²·k)→O(N²) FastPAM1 decomposition buys only ~2–8×, NOT k×.** Both the naive nested-loop swap and the O(N)-decomposition (FastPAM1 `ΔTD = acc + ploss[m]`) do the same N² distance-matrix LOOKUPS per iteration; the decomposition only removes the naive's cheap per-point O(k) *arithmetic*. So the speedup is the arithmetic-vs-memory ratio: ~3× at k=10 (memory dominates), growing to ~8× at k=200 (arithmetic finally rivals the lookup). Hoisting the per-candidate `ploss` heap alloc into a reused scratch buffer moved the bench <noise — confirming memory, not allocation, is the wall. A registered "≥10× swap speedup" band calibrated against arithmetic-bound *naive PAM* does NOT survive contact with a cached matrix (`.claude/baselines/2026-07-08-faster-pam-bench.md`). FasterPAM's real edge is orthogonal: it converges in ~1 eager SWEEP vs the best-swap variants' O(k) iterations (fewer *passes* over the matrix), but it is sequential, so parallel FastPAM1 still wins wall-time at large N.
...
- **A "structure" test (label ranges, sizes, converged-flag) does NOT test OPTIMALITY — a wrong answer can satisfy it.** The FastPAM k=1 unit test only asserted `labels all == 0` and `converged`, so it passed even when the new decomposition returned the BUILD medoid (a NaN bug, [[c-performance]]) instead of the optimum. What caught it was a DOWNSTREAM test with an *exact-value oracle*: `unit_test_fast_clara:619` reproduces the k=1 median bit-for-bit and asserted the wrong medoid. Lesson: for every algorithm, at least one test must pin the OPTIMAL output against an independent oracle (brute force / closed form), not just its shape. The brute-force local-optimality arbiter (no improving swap by full reassignment) added in `unit_test_faster_pam.cpp` is the general form — it is tie-independent and would have caught the NaN at any k.
