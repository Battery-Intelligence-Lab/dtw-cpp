> **ARCHIVED 2026-09-21.** This was the root `PLAN.md` (Research & Release Campaign, phases R0-R7).
> It is superseded by `.claude/PLAN.md`, which carries its open work forward (derivations D4-D18, the
> open F-findings, R5/R6/R7) and by `.claude/DECISIONS.md` (killed ideas, binding decisions).
> Kept verbatim below because `scripts/check_docs_contract.py` and `scripts/check_record_hygiene.py`
> pin exact strings in it, and because the F-/D-numbers are public in the docs. Do not append here.
> Evidence files it cites that are no longer in the tree are recoverable at commit `9c08074`.
