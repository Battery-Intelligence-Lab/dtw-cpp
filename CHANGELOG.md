...
- No-silent-fallback: requesting `use_gpu=true` on a HiGHS built without `CUPDLP_GPU` warns to stderr and runs on CPU with `gpu_used=false` (never a silent GPU claim); a future `CUPDLP_GPU=ON` build defines `DTWC_HIGHS_GPU` and flips `gpu_used=true`. GPU enablement itself is a separate follow-on step.
