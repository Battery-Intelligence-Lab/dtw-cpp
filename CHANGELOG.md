...
- **Change (tests, build):** every CTest entry is now gated by an execution
  ...
  `MAY_SKIP`, and requires Catch2's summary to report at least the assertion
  and test-case counts measured in `tests/floors.cmake` (generated by
  `scripts/measure_test_floors.py` from the four W0 `ctest -V` baselines).
...
- Vendored **nanoarrow 0.8.0** (Apache-2.0, two files `dtwc/extern/nanoarrow/nanoarrow.{h,c}`, no build-time fetch), symbol-namespaced `NANOARROW_NAMESPACE=DtwcNanoarrow` to avoid ODR clashes with any other Arrow library loaded in-process. Compiled into `dtwc++` behind a PRIVATE include — never leaks to consumers.
- New `dtwc::io::data_from_arrow` / `data_from_arrow_stream` (dtwc/io/arrow_c_data.{hpp,cpp}): build a `Data` from the Arrow C Data interface — a `list`/`large_list` of float32/float64 (each element → one univariate series) or a struct containing one (+ optional utf8 name child), and from the `__arrow_c_stream__` batch protocol. Public surface uses forward-declared C structs (nanoarrow stays encapsulated). Nulls / non-floating child / non-list top-level are rejected with `InvalidInput`. Values are copied once into owning storage; borrowed Arrow buffers are never aliased (caller may release immediately).
- Python: `dtwcpp.data_from_arrow_c_array(obj)` consumes the PyCapsule from `__arrow_c_array__` (pyarrow/DuckDB single array) or `__arrow_c_stream__` (polars/pandas batch stream), returning a `Data` — **without pyarrow**. `DTWClustering.fit`/`_prepare_data` auto-detect either dunder, so `DTWClustering(...).fit(polars_series)` works directly.
- Verified. C++ `tests/unit/io/test_arrow_c_data.cpp` (producer = nanoarrow builder, an independent path from the reader): 7 cases / 75 assertions — round-trip exactness (double + float32), name override, empty, and the three rejection paths. Python polars gate (independent producer, **pyarrow forcibly blocked** via `sys.modules['pyarrow']=None`): reader p_vec == polars series exactly (f64 + f32); `DTWClustering(k=2).fit_predict(polars Series)` recovers 2 separated clusters; pyarrow never importable across the whole run. Full ctest **95 → 96** (+1 suite), Python **391 passed / 10 skipped**, no regression. Run-log: `.claude/baselines/2026-07-09-arrow-ingest.md`.
- TC-DTW lower bound (arXiv:2101.07731, plan "if time") deferred — a pruning bound orthogonal to this ingest deliverable.

### Fixed (Phase 5 prerequisite — core builds without the optional llfio dependency)

- `-DDTWC_ENABLE_LLFIO=OFF` (the sandbox/no-optional-deps escape hatch) was broken: `dtwc/core/mmap_distance_matrix.hpp` and `mmap_data_store.hpp` `#include <llfio/...>` **unconditionally**, so any translation unit that pulls in `Problem.hpp` failed to compile without llfio — a violation of the "core must build without optional deps" rule (the earlier work only ever verified a `configure`-only run, never a compile). Guarded the llfio-touching members behind the existing `DTWC_HAS_MMAP` define: `MmapDistanceMatrix` stays a **complete** type in both configs (so `Problem::distMat_t` variant and all `std::visit`/`std::get` sites are untouched); only its file-mapping ctor/`open()`/`sync()` are compiled out, replaced by throwing stubs (no silent degradation). `MmapDataStore` is compiled out entirely; the CLI `.dtws` path throws a clear "rebuild with llfio" error. Verified: a fresh `-DDTWC_ENABLE_LLFIO=OFF` build compiles + links `dtwc++`/CLI clean; the llfio-ON gate stays digit-identical (96/96). This unblocked the Python wheel build, and with it the previously-deferred **5.5/5.6 Python runtime parity** (MSM/TWE/`mv_mode` now runtime-verified). Known-OPEN: the Windows wheel *with llfio ON* still fails on a backslash bug in the quickcpplib ninja-propagation patch (`pyproject.toml` left unchanged so Linux/CI wheels keep llfio; the local gating wheel uses `DTWC_ENABLE_LLFIO=OFF`).
...
- New tighter DTW lower bounds in `dtwc/core/lower_bound_impl.hpp`, templated on the pointwise metric (`L1Metric` + `SquaredL2Metric`): **`lb_enhanced`** (Tan, Petitjean & Webb, *SDM 2019*, elastic bands + LB_Keogh middle) and the historically named **`lb_webb`** / `lb_webb_symmetric`. The latter implements Webb & Petitjean's all-index `LB_Webb_NoLR` bridge and corrections plus a conservative trailing-flag cap; it is not full Algorithm 2 with `MinLRPaths`. The local directional bound is always at least matching-direction Keogh for the supported costs, and only the separate tail cap is proved no greater than exact-predicate NoLR. No universal ordering with full Webb is claimed. New `WebbEnvelope` (U, L, plus secondary `LU=L(U)`, `UL=U(L)`) built by reusing `compute_envelopes`. Wired into `LowerBoundStrategy { …, Enhanced, Webb }` and the Problem pruned cascade as selectable primitives.
...
- **Validity [HARD] → CONFIRMED.** `LB_Enhanced ≤ DTW_w` and the local NoLR-plus-tail-cap bound `≤ DTW_w` (L1 and SquaredL2) over random + adversarial inputs (shared endpoints, query-just-outside-envelope, constant, extreme 1e12), bands {0,1,2,5,10,20, 10%-of-n}, lengths incl. edge {2..11} and n≈2V; non-negativity and `LB(x,x)=0`. Provable and asserted: the local directional Webb bound is at least matching-direction Keogh (bridge plus non-negative Theorem-2 corrections; symmetric maximum therefore dominates symmetric Keogh). Effective `V=1` Enhanced dominates matching-direction Keogh; for effective `V>=2`, later exact repository witnesses establish both strict order directions, so the cascade takes the maximum. Test `tests/unit/adversarial/test_lb_enhanced_webb.cpp`: **39273 assertions / 14 cases**. Pruned + {Enhanced, Webb, Keogh} give a **digit-identical** matrix vs BruteForce (`unit_test_pruned_distance_matrix.cpp [strategy]`).
...
### Added (Phase 5 · Task 5.6 — independent multivariate DTW)

- New **`dtwc::dtw_independent_mv<T>`** (dtwc/warping.hpp): independent multivariate DTW (**DTW_I**; Shokoohi-Yekta et al., DMKD 31(1) 2017) — an independent univariate DTW per channel, summed (`DTW_I = Σ_c DTW(x[:,c], y[:,c])`). Distinct from the existing dependent **DTW_D** (`dtwFull_L_mv`/`dtwBanded_mv`), which forces one warping path shared by all channels. Neither mode dominates for clustering accuracy (the paper's central finding) — both are provided. De-interleaves the interleaved MV layout channel-by-channel into `thread_local` scratch, then calls the univariate kernel (exact EAPruned `dtwFull_eap` when unbanded, `dtwBanded` when banded); L1 / SquaredL2.
- Selected by a new orthogonal **`core::MVMode { Dependent, Independent }`** + `DTWVariantParams::mv_mode` (default **Dependent** → zero regression). **Wiring (CasADi-style parity):** dispatch interception at the top of `resolve_dtw_fn` (`make_independent`); CLI `--mv-mode dependent|independent` (+ TOML key); Python `MVMode` enum, `DTWVariantParams.mv_mode`, and the `KMedoids(mv_mode=...)` wrapper. (Checkpoint needs no change — it serialises only the variant name.)
- **v1 scope (documented, not silent):** Independent mode is Standard-variant + `MissingStrategy::Error` only. Any other combination is **rejected at bind time** — `set_variant()` rebinds eagerly, so the `InvalidInput` fires there (serial, before any parallel fill), never silently collapsed to dependent mode. Univariate (`ndim==1`) ignores the flag (falls through to the scalar Standard path).
- **BAND-ORACLE [HARD] → CONFIRMED:** `dtw_independent_mv(SquaredL2)` == aeon 1.5.0 `Σ_c dtw_distance(channel_c)` to rel ≤ 1e-9 on 10 non-degenerate 2-/3-channel pairs. **BAND-INEQ [HARD] → CONFIRMED:** `DTW_I ≤ DTW_D` on 200 random MV pairs (L1 + SquaredL2, unbanded + banded) — independent-math arbiter (each channel free to pick its own path). **BAND-WIRING / BAND-REJECT [HARD] → CONFIRMED.** Test `tests/unit/core/unit_test_independent_mv.cpp`: **7 cases, 889 assertions**. Full gate **95/95** (baseline 94 → +1 suite), no regression. `.claude/baselines/2026-07-08-independent-mv.md`.
- **Deferred:** the TC-DTW multivariate lower bound (arXiv:2101.07731, plan "if time") — a bound-tightening for the pruning/NN path (Tasks 5.2/5.3), orthogonal to this distance deliverable. Python runtime parity for `mv_mode` shares the same pre-existing wheel-build limitation as Task 5.5.
