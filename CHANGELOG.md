...
- No-silent-fallback: requesting `use_gpu=true` on a HiGHS built without `CUPDLP_GPU` warns to stderr and runs on CPU with `gpu_used=false` (never a silent GPU claim). `dtwc::mip::pdlp_gpu_available()` is a runtime capability query (the library reports its own build, since the compile define does not reach consumer TUs).
