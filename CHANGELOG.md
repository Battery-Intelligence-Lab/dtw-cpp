...
- Tuning: CFM subgradient deflection (damps zig-zag) + throttled primal polish (cheap O(Nk) assignment every iter, full O(N²) medoid polish every 16). Net win on the LR-core-vs-compact-MIP bench (identical optima, MIP scales ~N^2.6 while LR stays 1–2 orders cheaper): e.g. N=200 certified in 352 iters vs the earlier 4000-iter cap. The primal is exactly optimal at every tested N; the dual optimality *certificate* stalls at ~5e-5 for N=800 (subgradient slow tail at the non-smooth dual optimum — raising iters 4000→12000 barely moves it), which is fine because LR-core hands tight certification to the exact finish (Benders/OSLP, Tasks 4.3/4.5); a bundle/cutting-plane dual is the principled tightener if standalone 1e-6 is ever needed.
